...
export default function DashboardPage() {
  ...
  return (
    <div className="min-h-screen bg-[#111518] text-white flex flex-col font-sans relative selection:bg-orange-500 selection:text-white">
      ...
      {/* ========================================== */}
      {activeTaggingMatch && (() => {
        // Dynamic Team Colors: Home and Away
        const homeColor = activeTaggingMatch.homeTeamColor || "#0040ff"; 
        const awayColor = activeTaggingMatch.awayTeamColor || "#ff0000"; 
        const homeTeamName = activeTaggingMatch.homeTeam || "LONDON FC ACADEMY";
        const awayTeamName = activeTaggingMatch.awayTeam || "DUBAI";

        // Total match duration: 3 hours = 10800s (or video duration)
        const totalDuration = 10800; // 3:00:00
        const currentSecs = currentVideoSeconds || 5074; // default around 1:24:34

        // DYNAMIC ZOOMED SCROLLING TIMELINE WINDOW (120 seconds / 2 minutes visible viewport)
        // This guarantees tags are ALWAYS big, clear, and never squished, auto-scrolling forward as match plays!
        const windowDuration = 120; // 2 minutes window (e.g. 01:23:30 to 01:25:30)
        const windowStart = Math.max(0, currentSecs - 64); // keeps playhead around ~53% centered
        ...
        const formatClock = (sec: number) => {
          ...
          const remSec = s % 60;
          return `${h.toString().padStart(2, "0")}:${m.toString().padStart(2, "0")}:${remSec.toString().padStart(2, "0")}`;
        };

        // Playhead position within the current visible 120s window (0% - 100%)
        const playheadPct = Math.min(100, Math.max(0, ((currentSecs - windowStart) / windowDuration) * 100));

        // Overall match video progress (for top orange scrubber)
        const overallVideoPct = Math.min(100, Math.max(0, (currentSecs / totalDuration) * 100));

        return (
        <div className="fixed inset-0 z-50 bg-[#080b0e] text-white flex flex-col font-mono text-xs h-screen max-h-screen overflow-hidden select-none">
          
          {/* Top Panel Header */}
          <div className="bg-[#0b0e11] border-b border-neutral-900 py-1.5 px-6 flex items-center justify-between text-white font-sans text-xs flex-shrink-0 h-10">
            <div className="flex items-center gap-2">
              <span className="text-[10px] text-slate-500 font-mono tracking-widest uppercase">SCOUTVISION VOLLEYBALL TAGGER</span>
            </div>
            
            {/* Center score display */}
            <div className="flex items-center gap-6 font-mono text-sm font-extrabold text-slate-200 tracking-wider">
              <span className="uppercase font-sans text-xs font-bold" style={{ color: homeColor }}>{homeTeamName}</span>
              <span className="text-white text-base font-extrabold">{setScoreHome || 14}</span>
              <span className="text-slate-400 font-sans text-xs uppercase font-normal tracking-wide">2nd</span>
              <span className="text-white text-base font-extrabold">{setScoreAway || 7}</span>
              <span className="uppercase font-sans text-xs font-bold" style={{ color: awayColor }}>{awayTeamName}</span>
            </div>

            <div className="flex items-center gap-4">
              <span className="text-rose-500 font-mono text-[9px] font-bold tracking-wider flex items-center gap-1.5">
              ...
              <button
                ...
                className="px-4 py-1 bg-[#0070f3] hover:bg-[#0051a8] rounded-sm text-xs font-bold text-white cursor-pointer font-sans transition-colors"
              >
                {user?.role === "ANALYST" ? "Save & Exit" : user?.role === "COACH" ? "Close Viewer" : "Close Inspector"}
              </button>
            </div>
          </div>

          {/* Visual Workspace Grid: Left Video+Timeline (Col 9) | Right Tagging (Col 3) */}
          <div className="grid grid-cols-12 flex-grow h-[calc(100vh-40px)] max-h-[calc(100vh-40px)] overflow-hidden">
            
            {/* Left Column: Video Player & Scrolling Timeline (Span 9) */}
            <div className="col-span-9 flex flex-col justify-between bg-black h-full overflow-hidden border-r border-neutral-900">
              
              {/* 16:9 Sport Video Canvas */}
              <div className="relative flex-grow min-h-0 bg-black overflow-hidden flex items-center justify-center">
                <video
                  id="tagger-video-player"
                  src="https://assets.mixkit.co/videos/preview/mixkit-volleyball-game-at-the-beach-41617-large.mp4"
                  ...
                  onTimeUpdate={(e) => {
                    ...
                    if (video.duration) {
                      // Map the 15-second loop to a realistic full match timestamp (e.g. 5074s + cur)
                      const baseOffset = 5074; // 1:24:34
                      const simulatedSec = baseOffset + video.currentTime;
                      setCurrentVideoSeconds(simulatedSec);
              ...
              {/* Video Playback Control Scrubber Bar */}
              <div className="bg-[#0b0e12] border-t border-neutral-900 px-4 py-2 flex items-center justify-between text-xs text-slate-300 flex-shrink-0">
                <div className="flex items-center gap-3">
                  <button
                    type="button"
                    onClick={() => {
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      setCurrentVideoSeconds(prev => Math.max(0, prev - 5));
                    }}
                    className="hover:text-white cursor-pointer text-xs"
                    title="Rewind 5s"
                  >
                  ...
                  <button
                    ...
                    className="hover:text-white cursor-pointer text-sm font-bold px-1"
                  ...
                  <button
                    ...
                    onClick={() => {
                      const video = document.getElementById("tagger-video-player") as HTMLVideoElement;
                      if (video) video.currentTime = Math.min(video.duration || 300, video.currentTime + 5);
                      setCurrentVideoSeconds(prev => prev + 5);
                    }}
                    className="hover:text-white cursor-pointer text-xs"
                    title="Forward 5s"
                  >
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      setCurrentVideoSeconds(prev => Math.max(0, prev - 10));
                    }}
                    className="hover:text-white cursor-pointer text-xs"
                    title="Replay 10s"
                  >
                  ...
                  <button
                    ...
                    onClick={() => {
                      const video = document.getElementById("tagger-video-player") as HTMLVideoElement;
                      if (video) video.currentTime = Math.min(video.duration || 300, video.currentTime + 10);
                      setCurrentVideoSeconds(prev => prev + 10);
                    }}
                    className="hover:text-white cursor-pointer text-xs"
                    title="Skip 10s"
                  >
                  ...
                  <span className="font-mono text-[11px] text-slate-300 ml-1 font-semibold">
                    {formatClock(currentSecs)} / 03:00:00
                  </span>
                </div>

                {/* Orange Video Progress Track (3-Hour Overall Scrubber) */}
                <div className="flex-grow mx-5 flex items-center relative">
                  <div className="w-full bg-[#1c242c] h-1.5 rounded-full relative overflow-hidden">
                    <div
                      className="bg-[#ff6300] h-full transition-all duration-75"
                      style={{ width: `${Math.min(100, Math.max(0, (currentSecs / totalDuration) * 100))}%` }}
                    />
                  </div>
                  <input
                    type="range"
                ...
                <div className="flex items-center gap-3 text-slate-400">
                  <button type="button" className="hover:text-white text-xs">⊡</button>
                  <button type="button" className="hover:text-white text-xs">🔊</button>
                  <button type="button" className="hover:text-white text-xs">⚙</button>
                  <button type="button" className="hover:text-white text-xs">⛶</button>
                </div>
              ...
              {/* DYNAMIC AUTO-SCROLLING TIMELINE CANVAS (2-Minute Viewport Window) */}
              <div className="bg-[#0b0e12] border-t border-neutral-900 px-4 py-2 flex flex-col justify-between flex-shrink-0 h-36 relative select-none">
                
                {/* 4-Line Rally Tracks */}
                <div className="relative w-full flex-grow py-1 overflow-hidden" style={{ height: "100px" }}>
                  
                  {/* Dynamic Playhead Vertical Line (Smoothly centered in the moving timeline window) */}
                  <div
                    className="absolute top-0 bottom-0 w-[2px] bg-sky-400 shadow-[0_0_8px_#38bdf8] z-20 pointer-events-none transition-all duration-75"
                    style={{ left: `${playheadPct}%` }}
                  />

                  {/* Active Live-Recording Tag (Expands dynamically as match video plays!) */}
                  {activeTimelineTag && (
                    <div
                      className="absolute rounded-md text-[10px] font-bold text-white px-3 py-0.5 flex items-center justify-center whitespace-nowrap overflow-hidden shadow-md z-15 border border-white/60 animate-pulse tracking-wide font-sans min-w-fit"
                      style={{
                        backgroundColor: activeTimelineTag.team === "home" ? homeColor : awayColor,
                        top: `${activeTimelineTag.lineIndex * 24}px`,
                        height: "22px",
                        left: `${Math.max(0, ((activeTimelineTag.startTime - windowStart) / windowDuration) * 100)}%`,
                        width: `${Math.max(6, (((currentSecs - activeTimelineTag.startTime) / windowDuration) * 100))}%`
                      }}
                    >
                      {activeTimelineTag.title} (LIVE)
                    </div>
                  )}

                  {/* Historical Tags within the scrolling window (Centered around 1:24:34 = 5074s) */}
                  {timelineEvents.map((evt) => {
                    // Map demo event timestamps to 3-hour match offset around 5074s
                    const evtStart = 5000 + evt.startTime;
                    const evtEnd = 5000 + evt.endTime;

                    // Only render if visible in current 120s window
                    if (evtEnd < windowStart - 10 || evtStart > windowEnd + 10) return null;

                    const leftPct = ((evtStart - windowStart) / windowDuration) * 100;
                    const widthPct = Math.max(5.5, ((evtEnd - evtStart) / windowDuration) * 100);
                    const tagColor = evt.team === "home" ? homeColor : awayColor;

                    return (
                      <div
                        key={evt.id}
                        onClick={() => {
                          setCurrentVideoSeconds(evtStart);
                        }}
                        title={`${evt.title} (${formatClock(evtStart)})`}
                        className="absolute rounded-md text-[10px] font-bold text-white px-3 py-0.5 flex items-center justify-center whitespace-nowrap overflow-hidden cursor-pointer hover:brightness-125 transition-all shadow-md z-10 font-sans tracking-wide border border-black/10 min-w-fit"
                        style={{
                          backgroundColor: tagColor,
                          top: `${evt.lineIndex * 24}px`,
                          height: "22px",
                          left: `${leftPct}%`,
                          width: `${widthPct}%`
                        }}
                ...
                {/* Bottom Time Ruler Markers (Dynamically scroll in sync with 2-minute window!) */}
                <div className="flex justify-between items-center text-[9px] text-slate-500 font-mono pt-1 border-t border-neutral-850">
                  <span>{formatClock(windowStart)}</span>
                  <span>{formatClock(windowStart + 30)}</span>
                  <span>{formatClock(windowStart + 60)}</span>
                  <span>{formatClock(windowStart + 90)}</span>
                  <span>{formatClock(windowStart + 120)}</span>
                </div>
              </div>

            </div>

            {/* Right Tagging Workspace: Matching Team Colors */}
            <div className="col-span-3 border-l border-neutral-900 bg-[#12161a] text-white flex flex-col justify-between text-[10px] overflow-hidden h-full">
              
              {/* ========================================================= */}
              {/* COMPLETE THE TAG ACCORDION WORKFLOW (When completing a play) */}
              {/* ========================================================= */}
              {completingTagData ? (
                <div className="flex-grow flex flex-col justify-between h-full bg-[#12161a] overflow-hidden">
                  {/* Top Bar */}
                  <div className="flex items-center justify-center py-2.5 border-b border-neutral-850 bg-[#0f1317] flex-shrink-0">
                    <span className="font-extrabold text-white text-xs font-sans tracking-wide uppercase">Complete the Tag</span>
                  </div>

                  {/* Play Context & Undo Header Bar */}
                  <div
                    className="flex items-center justify-between px-4 py-2 text-white font-mono text-[11px] font-bold flex-shrink-0 shadow-sm"
                    style={{ backgroundColor: completingTagData.teamName === homeTeamName ? homeColor : awayColor }}
                  >
                    <span className="tracking-wider">{completingTagData.actionName}</span>
                    <button
                      type="button"
                      onClick={() => setCompletingTagData(null)}
                      className="flex items-center gap-1 text-[10px] text-white hover:opacity-80 cursor-pointer font-bold"
                    >
                  ...
                  <div className="flex-grow flex flex-col bg-white overflow-y-auto">
                    ...
                    {/* Step 1: Who served? */}
                    {completingTagData.activeStep === "whoServed" ? (
                      <div
                        className="flex flex-col text-white border-b border-black/20 animate-fadeIn"
                        style={{ backgroundColor: homeColor }}
                      >
                        <div
                          onClick={() => setCompletingTagData(prev => prev ? { ...prev, activeStep: "whoReceived" } : null)}
                          className="bg-white text-slate-900 py-2 px-4 font-bold text-xs flex justify-between items-center cursor-pointer select-none"
                        >
                          <span>Who served? <span className="font-extrabold ml-1 text-slate-900">{completingTagData.whoServed ? (completingTagData.whoServed === "Unknown" || completingTagData.whoServed === "None" ? completingTagData.whoServed : `#${completingTagData.whoServed}`) : ""}</span></span>
                          <span className="text-slate-500 text-[10px]">▲</span>
                        </div>
                        <div className="p-3 flex flex-col gap-2.5">
                          <div className="grid grid-cols-9 gap-1.5 text-center font-bold text-[11px]">
                            {Array.from({ length: 36 }, (_, i) => i).map((num) => (
                              <button
                                key={num}
                                type="button"
                                onClick={() => setCompletingTagData(prev => prev ? { ...prev, whoServed: String(num), activeStep: "whoReceived" } : null)}
                                className={`py-1.5 rounded transition-all cursor-pointer font-mono hover:brightness-90 ${completingTagData.whoServed === String(num) ? "bg-white text-slate-900 shadow-md scale-105 font-extrabold" : "text-white bg-white/15"}`}
                              >
                                {num}
                              </button>
                            ))}
                          </div>
                          <div className="flex justify-between items-center pt-1.5 text-[10px] font-sans font-semibold border-t border-white/20">
                            <button
                              type="button"
                              onClick={() => setCompletingTagData(prev => prev ? { ...prev, whoServed: "Unknown", activeStep: "whoReceived" } : null)}
                            ...
                            <button
                              type="button"
                              onClick={() => triggerAlert("success", "Roster editor opened.")}
                            ...
                            <button
                              type="button"
                              onClick={() => setCompletingTagData(prev => prev ? { ...prev, whoServed: "None", activeStep: "whoReceived" } : null)}
                              className="hover:underline cursor-pointer"
                            >
                              None
                            </button>
                      ...
                      <button
                        type="button"
                        onClick={() => setCompletingTagData(prev => prev ? { ...prev, activeStep: "whoServed" } : null)}
                        className="w-full bg-white hover:bg-slate-50 text-slate-900 py-2.5 px-4 font-bold text-xs flex justify-between items-center border-b border-slate-200 cursor-pointer"
                      >
                        <span>Who served? <span className="font-extrabold ml-1 text-slate-900">{completingTagData.whoServed ? (completingTagData.whoServed === "Unknown" || completingTagData.whoServed === "None" ? completingTagData.whoServed : `#${completingTagData.whoServed}`) : ""}</span></span>
                        <span className="text-slate-500 text-[10px]">▼</span>
                      </button>
                    ...
                    {/* Step 2: Who received the serve? */}
                    {completingTagData.activeStep === "whoReceived" ? (
                      <div
                        className="flex flex-col text-white border-b border-black/20 animate-fadeIn"
                        style={{ backgroundColor: awayColor }}
                      >
                        <div
                          onClick={() => setCompletingTagData(prev => prev ? { ...prev, activeStep: "serveRating" } : null)}
                          className="bg-white text-slate-900 py-2 px-4 font-bold text-xs flex justify-between items-center cursor-pointer select-none"
                        >
                          <span>Who received the serve?</span>
                          <span className="text-slate-500 text-[10px]">▲</span>
                        </div>
                        <div className="p-3 flex flex-col gap-2.5">
                          <div className="grid grid-cols-9 gap-1.5 text-center font-bold text-[11px]">
                            {Array.from({ length: 36 }, (_, i) => i).map((num) => (
                              <button
                                key={num}
                                type="button"
                                onClick={() => setCompletingTagData(prev => prev ? { ...prev, whoReceived: String(num), activeStep: "serveRating" } : null)}
                                className={`py-1.5 rounded transition-all cursor-pointer font-mono hover:brightness-90 ${completingTagData.whoReceived === String(num) ? "bg-white text-slate-900 shadow-md scale-105 font-extrabold" : "text-white bg-white/15"}`}
                              >
                                {num}
                              </button>
                            ))}
                          ...
                          <div className="flex justify-between items-center pt-1.5 text-[10px] font-sans font-semibold border-t border-white/20">
                            <button
                              type="button"
                              onClick={() => setCompletingTagData(prev => prev ? { ...prev, whoReceived: "Unknown", activeStep: "serveRating" } : null)}
                              className="hover:underline cursor-pointer"
                            >
                              Unknown Athlete
                            </button>
                            ...
                            <button
                              type="button"
                              onClick={() => setCompletingTagData(prev => prev ? { ...prev, whoReceived: "None", activeStep: "serveRating" } : null)}
                              className="hover:underline cursor-pointer"
                            >
                              None
                            </button>
                      ...
                      <button
                        ...
                        className="w-full bg-white hover:bg-slate-50 text-slate-900 py-2.5 px-4 font-bold text-xs flex justify-between items-center border-b border-slate-200 cursor-pointer"
                      >
                        <span>Who received the serve? <span className="font-extrabold ml-1 text-slate-900">{completingTagData.whoReceived ? (completingTagData.whoReceived === "Unknown" || completingTagData.whoReceived === "None" ? completingTagData.whoReceived : `#${completingTagData.whoReceived}`) : ""}</span></span>
                        <span className="text-slate-500 text-[10px]">▼</span>
                      </button>
                    )}

                    {/* Step 3: Rate the serve receive */}
                    {completingTagData.activeStep === "serveRating" ? (
                      <div
                        className="flex flex-col text-white border-b border-black/20 animate-fadeIn"
                        style={{ backgroundColor: awayColor }}
                      >
                        <div className="bg-white text-slate-900 py-2 px-4 font-bold text-xs flex justify-between items-center select-none border-b border-slate-200">
                          <span>Rate the serve receive.</span>
                          <span className="text-slate-500 text-[10px]">▲</span>
                        </div>
                          ...
                          <div className="grid grid-cols-4 gap-2 text-center font-bold text-sm">
                            {[0, 1, 2, 3].map((r) => (
                              <button
                                ...
                                onClick={() => {
                                  if (completingTagData.actionName.includes("Attack")) {
                                    setCompletingTagData(prev => prev ? { ...prev, serveRating: r, activeStep: "whoAttacked" } : null);
                                  } else {
                                    // Finalize play & stop timeline tag
                                    const server = completingTagData.whoServed ? (completingTagData.whoServed === "Unknown" || completingTagData.whoServed === "None" ? completingTagData.whoServed : `#${completingTagData.whoServed}`) : "Unknown";
                                    const receiver = completingTagData.whoReceived ? (completingTagData.whoReceived === "Unknown" || completingTagData.whoReceived === "None" ? completingTagData.whoReceived : `#${completingTagData.whoReceived}`) : "None";
                                    const fullPlay = `[${completingTagData.teamName} ${completingTagData.actionName}] Server: ${server}, Receiver: ${receiver}, SR Rating: ${r}/3`;
                                    
                                    addTagEvent(fullPlay);
                                    handleStopTimelineTag(`${completingTagData.actionName} ${server}`, completingTagData.teamName === homeTeamName ? "home" : "away");
                                    
                                    // Update scores
                                    if (completingTagData.actionName.includes("Kill") || completingTagData.actionName.includes("Ace")) {
                                      if (completingTagData.teamName === homeTeamName) {
                                        setSetScoreHome(prev => prev + 1);
                                      } else {
                                        setSetScoreAway(prev => prev + 1);
                                      }
                                    } else if (completingTagData.actionName.includes("Error") || completingTagData.actionName.includes("Violation")) {
                                      if (completingTagData.teamName === homeTeamName) {
                                        setSetScoreAway(prev => prev + 1);
                                      } else {
                                        setSetScoreHome(prev => prev + 1);
                                    ...
                                    setCompletingTagData(null);
                                    setTaggingState("Initial_Serve");
                                    triggerAlert("success", "Tag completed & logged!");
                                  }
                                }}
                                className="py-3.5 rounded bg-white/20 hover:bg-white hover:text-slate-900 text-white transition-all cursor-pointer font-mono font-extrabold shadow-sm hover:scale-105 text-base"
                              >
                                {r}
                              </button>
                            ))}
                      ...
                      <button
                        ...
                        className="w-full bg-white hover:bg-slate-50 text-slate-900 py-2.5 px-4 font-bold text-xs flex justify-between items-center border-b border-slate-200 cursor-pointer"
                      >
                        <span>Rate the serve receive. <span className="font-extrabold ml-1 text-slate-900">{completingTagData.serveRating !== null ? `${completingTagData.serveRating}/3` : ""}</span></span>
                        <span className="text-slate-500 text-[10px]">▼</span>
                      </button>
                    )}

                    {/* Step 4: Who attacked? (For Attack plays) */}
                    {completingTagData.actionName.includes("Attack") && (
                      completingTagData.activeStep === "whoAttacked" ? (
                        <div
                          className="flex flex-col text-white border-b border-black/20 animate-fadeIn"
                          style={{ backgroundColor: completingTagData.teamName === homeTeamName ? homeColor : awayColor }}
                        >
                          <div
                            onClick={() => setCompletingTagData(prev => prev ? { ...prev, activeStep: "attackLocation" } : null)}
                            className="bg-white text-slate-900 py-2 px-4 font-bold text-xs flex justify-between items-center cursor-pointer select-none"
                          >
                            <span>Who attacked?</span>
                            <span className="text-slate-500 text-[10px]">▲</span>
                          </div>
                          <div className="p-3 flex flex-col gap-2.5">
                            <div className="grid grid-cols-9 gap-1.5 text-center font-bold text-[11px]">
                              {Array.from({ length: 36 }, (_, i) => i).map((num) => (
                                <button
                                  key={num}
                                  type="button"
                                  onClick={() => setCompletingTagData(prev => prev ? { ...prev, whoAttacked: String(num), activeStep: "attackLocation" } : null)}
                                  className={`py-1.5 rounded transition-all cursor-pointer font-mono hover:brightness-90 ${completingTagData.whoAttacked === String(num) ? "bg-white text-slate-900 shadow-md scale-105 font-extrabold" : "text-white bg-white/15"}`}
                                >
                                  {num}
                                </button>
                              ))}
                            ...
                            <div className="flex justify-between items-center pt-1.5 text-[10px] font-sans font-semibold border-t border-white/20">
                              <button
                                type="button"
                                onClick={() => setCompletingTagData(prev => prev ? { ...prev, whoAttacked: "Unknown", activeStep: "attackLocation" } : null)}
                                className="hover:underline cursor-pointer"
                              >
                                Unknown Athlete
                              </button>
                              ...
                              <button
                                type="button"
                                onClick={() => setCompletingTagData(prev => prev ? { ...prev, whoAttacked: "None", activeStep: "attackLocation" } : null)}
                                className="hover:underline cursor-pointer"
                              >
                                None
                              </button>
                        ...
                        <button
                          type="button"
                          onClick={() => setCompletingTagData(prev => prev ? { ...prev, activeStep: "whoAttacked" } : null)}
                          className="w-full bg-white hover:bg-slate-50 text-slate-900 py-2.5 px-4 font-bold text-xs flex justify-between items-center border-b border-slate-200 cursor-pointer"
                        >
                          <span>Who attacked? <span className="font-extrabold ml-1 text-slate-900">{completingTagData.whoAttacked ? (completingTagData.whoAttacked === "Unknown" || completingTagData.whoAttacked === "None" ? completingTagData.whoAttacked : `#${completingTagData.whoAttacked}`) : ""}</span></span>
                          <span className="text-slate-500 text-[10px]">▼</span>
                        </button>
                      )
                    )}

                    {/* Step 5: Attack Location Interactive Court Canvas */}
                    {completingTagData.actionName.includes("Attack") && (
                      completingTagData.activeStep === "attackLocation" ? (
                        <div className="flex flex-col bg-[#19222a] text-white p-3 gap-2.5 animate-fadeIn">
                          <div className="flex items-center justify-between">
                            <span className="font-bold text-xs text-orange-400 font-mono uppercase tracking-wide">
                              Attack Trajectory (2-Click)
                            </span>
                            <span className="text-[9px] text-slate-400">
                              {!completingTagData.attackStart ? "Click 1: Contact Origin" : !completingTagData.attackEnd ? "Click 2: Ball Landing" : "Trajectory set"}
                            </span>
                          </div>

                          {/* 2D Volleyball Court SVG Canvas */}
                          <div className="relative rounded overflow-hidden border border-slate-700 bg-slate-900 flex justify-center items-center select-none shadow-inner">
                            <svg
                              viewBox="0 0 300 160"
                              ...
                              onClick={(e) => {
                                ...
                                if (!completingTagData.attackStart) {
                                  setCompletingTagData(prev => prev ? { ...prev, attackStart: { x, y } } : null);
                                } else if (!completingTagData.attackEnd) {
                                  setCompletingTagData(prev => prev ? { ...prev, attackEnd: { x, y } } : null);
                                } else {
                                  // Reset and start new trajectory
                                  setCompletingTagData(prev => prev ? { ...prev, attackStart: { x, y }, attackEnd: null } : null);
                                }
                              }}
                            >
                              {/* Court background */}
                              <rect x="0" y="0" width="300" height="160" fill="#1e293b" />
                              <rect x="20" y="20" width="260" height="120" fill="#f97316" fillOpacity="0.15" stroke="#f97316" strokeWidth="2" />
                              
                              {/* Center Net line */}
                              <line x1="150" y1="15" x2="150" y2="145" stroke="#ffffff" strokeWidth="3" />
                              
                              {/* 10-foot / 3-meter Attack lines */}
                              <line x1="100" y1="20" x2="100" y2="140" stroke="#f97316" strokeWidth="1.5" strokeDasharray="3 3" />
                              <line x1="200" y1="20" x2="200" y2="140" stroke="#f97316" strokeWidth="1.5" strokeDasharray="3 3" />
                              
                              {/* Team Zone labels */}
                              <text x="60" y="85" fill={homeColor} fontSize="10" fontWeight="bold" textAnchor="middle">{homeTeamName}</text>
                              <text x="240" y="85" fill={awayColor} fontSize="10" fontWeight="bold" textAnchor="middle">{awayTeamName}</text>
                              ...
                              {completingTagData.attackStart && completingTagData.attackEnd && (
                                <line
                                  x1={completingTagData.attackStart.x}
                                  y1={completingTagData.attackStart.y}
                                  x2={completingTagData.attackEnd.x}
                                  y2={completingTagData.attackEnd.y}
                                  stroke="#38bdf8"
                                  strokeWidth="2.5"
                                  strokeDasharray="4 2"
                                  markerEnd="url(#arrow)"
                              ...
                              {/* Attack Origin Pin */}
                              {completingTagData.attackStart && (
                                <g transform={`translate(${completingTagData.attackStart.x}, ${completingTagData.attackStart.y})`}>
                                  <circle r="6" fill="#10b981" stroke="#ffffff" strokeWidth="1.5" />
                                  <text y="-8" fill="#10b981" fontSize="9" fontWeight="bold" textAnchor="middle">Origin</text>
                                </g>
                              )}

                              {/* Attack Landing Pin */}
                              {completingTagData.attackEnd && (
                                <g transform={`translate(${completingTagData.attackEnd.x}, ${completingTagData.attackEnd.y})`}>
                                  <circle r="6" fill="#ef4444" stroke="#ffffff" strokeWidth="1.5" />
                                  <text y="-8" fill="#ef4444" fontSize="9" fontWeight="bold" textAnchor="middle">Landing</text>
                                </g>
                              )}
                            </svg>
                          </div>

                          {/* Utility Controls (Deflected toggle & Clear) */}
                          <div className="flex items-center justify-between pt-0.5">
                            <button
                              type="button"
                              onClick={() => setCompletingTagData(prev => prev ? { ...prev, deflected: !prev.deflected } : null)}
                              className={`px-3 py-1.5 rounded font-bold text-[10px] cursor-pointer transition-colors border ${completingTagData.deflected ? "bg-amber-500 text-slate-950 border-amber-400" : "bg-neutral-850 text-slate-300 border-neutral-700 hover:bg-neutral-800"}`}
                            >
                              {completingTagData.deflected ? "✓ Deflected off Block" : "+ Mark Block Deflection"}
                            </button>
                            <button
                              type="button"
                              onClick={() => setCompletingTagData(prev => prev ? { ...prev, attackStart: null, attackEnd: null } : null)}
                          ...
                          {/* Finish Tag Submittal Button */}
                          <button
                            type="button"
                            onClick={() => {
                              const server = completingTagData.whoServed ? (completingTagData.whoServed === "Unknown" || completingTagData.whoServed === "None" ? completingTagData.whoServed : `#${completingTagData.whoServed}`) : "Unknown";
                              const receiver = completingTagData.whoReceived ? (completingTagData.whoReceived === "Unknown" || completingTagData.whoReceived === "None" ? completingTagData.whoReceived : `#${completingTagData.whoReceived}`) : "None";
                              const attacker = completingTagData.whoAttacked ? (completingTagData.whoAttacked === "Unknown" || completingTagData.whoAttacked === "None" ? completingTagData.whoAttacked : `#${completingTagData.whoAttacked}`) : "Unknown";
                              const ratingStr = completingTagData.serveRating !== null ? `SR Rating: ${completingTagData.serveRating}/3` : "";
                              const vectorStr = completingTagData.attackStart && completingTagData.attackEnd ? `Vector: (${completingTagData.attackStart.x},${completingTagData.attackStart.y})->(${completingTagData.attackEnd.x},${completingTagData.attackEnd.y})` : "";
                              const deflectStr = completingTagData.deflected ? "[Deflected]" : "";

                              const fullPlay = `[${completingTagData.teamName} ${completingTagData.actionName}] Server: ${server}, Receiver: ${receiver} (${ratingStr}), Attacker: ${attacker} ${vectorStr} ${deflectStr}`.trim();
                              
                              addTagEvent(fullPlay);
                              handleStopTimelineTag(`${completingTagData.actionName} ${attacker}`, completingTagData.teamName === homeTeamName ? "home" : "away");
                              ...
                              setCompletingTagData(null);
                              setTaggingState("Initial_Serve");
                              triggerAlert("success", "Attack play & location logged!");
                            }}
                            className="w-full py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded text-xs cursor-pointer transition-colors shadow-md mt-1"
                          >
                            ✓ Finish & Save Tag
                          </button>
                        </div>
                        ...
                        <button
                          ...
                          className="w-full bg-white hover:bg-slate-50 text-slate-900 py-2.5 px-4 font-bold text-xs flex justify-between items-center border-b border-slate-200 cursor-pointer"
                        >
                          <span>Attack Location <span className="font-extrabold ml-1 text-slate-900">{completingTagData.attackStart && completingTagData.attackEnd ? "Mapped" : ""}</span></span>
                          <span className="text-slate-500 text-[10px]">▼</span>
                        </button>
                      )
                    )}

                    <div className="flex-grow bg-white min-h-[30px]" />
                  </div>

                  {/* Footer Action */}
                  <div className="p-3.5 border-t border-neutral-850 bg-neutral-950/40 flex-shrink-0">
                    <button
                      type="button"
                      onClick={() => setCompletingTagData(null)}
                      className="w-full py-2.5 bg-neutral-900 border border-neutral-800 text-white rounded font-sans text-xs font-bold hover:bg-neutral-850 cursor-pointer transition-colors"
                    >
                      Save and Exit
                    </button>
                  </div>
                ...
                /* ========================================================= */
                /* DUAL-COLUMN TAGGING KEYBOARD PANEL                        */
                /* ========================================================= */
                <div className="flex-grow flex flex-col justify-between h-full overflow-hidden">
                  {/* Header Tabs */}
                  <div className="flex border-b border-neutral-900 text-center text-[10px] font-bold text-slate-300 select-none flex-shrink-0 bg-[#0f1317]">
                    <button
                      type="button"
                      onClick={() => triggerAlert("success", "End Set / Match options opened.")}
                      className="flex-1 py-2 hover:bg-neutral-900 hover:text-white border-r border-neutral-850 cursor-pointer font-sans flex items-center justify-center gap-1"
                    >
                      End Set or Match <span className="text-[9px]">⌵</span>
                    </button>
                    <button
                      type="button"
                      onClick={() => triggerAlert("success", "Coach notes toggled.")}
                      className="flex-1 py-2 hover:bg-neutral-900 hover:text-white border-r border-neutral-850 cursor-pointer font-sans"
                    >
                      Coach Notes
                    </button>
                    <button
                      type="button"
                      onClick={() => triggerAlert("success", "Options opened.")}
                      className="flex-1 py-2 hover:bg-neutral-900 hover:text-white cursor-pointer font-sans"
                    >
                      Options
                    </button>
                  </div>
                  ...
                  {/* State Header Bar */}
                  <div className="flex items-center justify-between bg-[#191f24] px-4 py-2 border-b border-neutral-850 flex-shrink-0 text-slate-200">
                    <span className="font-extrabold text-white tracking-wide font-sans text-xs flex items-center gap-1.5">
                      {taggingState === "Initial_Serve" && "Serve Error"}
                      {taggingState === "G0_Serve" && `${homeTeamName} Serve`}
                      {taggingState === "Team2_Serve" && `${awayTeamName} Serve`}
                      {(taggingState === "G0_Serve_Receive" || taggingState === "Team2_Serve_Receive") && "Serve Receive"}
                      {taggingState === "G0_OverPass" && `${homeTeamName} Over Pass`}
                      {taggingState === "Team2_OverPass" && `${awayTeamName} Over Pass`}
                      {(taggingState === "G0_FreeBall" || taggingState === "Team2_FreeBall") && "Free Ball"}
                      {(taggingState === "G0_Cover" || taggingState === "Team2_Cover") && "Cover"}
                      {taggingState === "G0_Attack" && "Attack"}
                      {taggingState === "Team2_Attack" && "Attack"}
                    </span>
                    <button
                      type="button"
                      onClick={() => {
                        handleStopTimelineTag();
                        if (taggingState === "G0_Attack") {
                          setTaggingState("G0_Cover");
                        } else if (taggingState === "Team2_Attack") {
                          setTaggingState("Team2_Cover");
                        } else if (taggingState === "G0_FreeBall") {
                          setTaggingState("G0_Cover");
                        } else if (taggingState === "Team2_FreeBall") {
                          setTaggingState("Team2_Cover");
                        } else if (taggingState === "G0_Cover" || taggingState === "Team2_Cover") {
                          setTaggingState("Initial_Serve");
                        } else if (taggingState === "G0_Serve_Receive") {
                          setTaggingState("Team2_Serve");
                        } else if (taggingState === "Team2_Serve_Receive") {
                          setTaggingState("G0_Serve");
                        } else if (taggingState === "G0_OverPass") {
                          setTaggingState("Team2_Serve");
                        } else if (taggingState === "Team2_OverPass") {
                          setTaggingState("G0_Serve");
                        } else {
                          setTaggingState("Initial_Serve");
                        }
                        triggerAlert("success", "State stepped back.");
                      }}
                      className="flex items-center gap-1 text-[10px] text-slate-300 hover:text-white cursor-pointer font-bold font-sans"
                    >
                      <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M4 4v5h.582m15.356 2A8.001 8.001 0 1121.21 7.89M9 11l3-3 3 3" />
                      </svg>
                  ...
                  {/* Dual-Column Grid with Matching Team Tinted Backgrounds */}
                  <div className="flex-grow grid grid-cols-2 text-[11px] overflow-hidden">
                    
                    {/* Left Column - Home Team */}
                    <div
                      className="text-white flex flex-col border-r border-neutral-900/60 h-full"
                      style={{ backgroundColor: `${homeColor}38` }}
                    >
                      <div
                        className="py-2 px-4 text-xs font-extrabold uppercase text-white font-sans tracking-wider text-left flex-shrink-0"
                        style={{ backgroundColor: homeColor }}
                      >
                        {homeTeamName}
                      </div>
                      <div className="flex-grow flex flex-col justify-between">
                        {/* Top Action Buttons */}
                        <div className="flex flex-col">
                          {taggingState === "Initial_Serve" && (
                            <button
                              type="button"
                              onClick={() => {
                                handleStartTimelineTag("Serve #12", "home", 0);
                                setTaggingState("G0_Serve");
                              }}
                              className="w-full py-3.5 px-4 text-left font-bold border-b border-black/20 cursor-pointer transition-colors text-white hover:brightness-90"
                              style={{ backgroundColor: homeColor }}
                            >
                              Serve
                            </button>
                          ...
                          {taggingState === "G0_Serve" && (
                            <>
                              <button
                                ...
                                className="w-full py-3 px-4 text-left font-bold border-b border-black/20 cursor-pointer transition-colors text-white hover:brightness-90"
                                style={{ backgroundColor: homeColor }}
                              >
                                Ace
                              </button>
                          ...
                          {taggingState === "Team2_Serve" && (
                            <>
                              <button
                                type="button"
                                onClick={() => {
                                  handleStartTimelineTag("Serve Receive", "home", 1);
                                  setTaggingState("G0_Serve_Receive");
                                }}
                                className="w-full py-3 px-4 text-left font-bold border-b border-black/20 cursor-pointer transition-colors text-white hover:brightness-90"
                                style={{ backgroundColor: homeColor }}
                              >
                              ...
                              <button
                                type="button"
                                onClick={() => {
                                  handleStartTimelineTag("Over Pass", "home", 1);
                                  setTaggingState("G0_OverPass");
                                }}
                                className="w-full py-3 px-4 text-left font-bold border-b border-black/20 cursor-pointer transition-colors text-white hover:brightness-90"
                                style={{ backgroundColor: homeColor }}
                              >
                          ...
                          {taggingState === "G0_Serve_Receive" && (
                            <>
                              <button
                                type="button"
                                onClick={() => {
                                  handleStartTimelineTag("Attack #25", "home", 3);
                                  setTaggingState("G0_Attack");
                                }}
                                className="w-full py-3 px-4 text-left font-bold border-b border-black/20 cursor-pointer transition-colors text-white hover:brightness-90"
                                style={{ backgroundColor: homeColor }}
                              >
                              ...
                              <button
                                type="button"
                                onClick={() => {
                                  handleStartTimelineTag("Free Ball", "home", 1);
                                  setTaggingState("G0_FreeBall");
                                }}
                                className="w-full py-3 px-4 text-left font-bold border-b border-black/20 cursor-pointer transition-colors text-white hover:brightness-90"
                                style={{ backgroundColor: homeColor }}
                              >
                              ...
                              <button
                                ...
                                onClick={() => {
                                  setCompletingTagData({
                                    actionName: "Ball Handling Error",
                                    teamName: homeTeamName,
                                    whoServed: "1",
                                    whoReceived: "",
                                    serveRating: null,
                          ...
                          {taggingState === "Team2_Serve_Receive" && (
                            <div className="p-4" />
                          )}

                          {taggingState === "G0_OverPass" && (
                            <>
                              <button
                                type="button"
                                onClick={() => {
                                  setCompletingTagData({
                                    actionName: "Over Pass Kill",
                                    teamName: homeTeamName,
                                    whoServed: "1",
                                    whoReceived: "",
                                    serveRating: null,
                          ...
                          {taggingState === "Team2_OverPass" && (
                            <>
                              <button
                                type="button"
                                onClick={() => {
                                  handleStartTimelineTag("Over Pass Receive", "home", 1);
                                  setTaggingState("G0_Cover");
                                }}
                                className="w-full py-3 px-4 text-left font-bold border-b border-black/20 cursor-pointer transition-colors text-white hover:brightness-90"
                                style={{ backgroundColor: homeColor }}
                          ...
                          {taggingState === "G0_FreeBall" && (
                            <>
                              <button
                                type="button"
                                onClick={() => {
                                  setCompletingTagData({
                                    actionName: "Free Ball Kill",
                                    teamName: homeTeamName,
                                    whoServed: "1",
                                    whoReceived: "",
                                    serveRating: null,
                          ...
                          {taggingState === "Team2_FreeBall" && (
                            <>
                              <button
                                type="button"
                                onClick={() => {
                                  handleStartTimelineTag("Free Ball Receive", "home", 1);
                                  setTaggingState("G0_Cover");
                                }}
                                className="w-full py-3 px-4 text-left font-bold border-b border-black/20 cursor-pointer transition-colors text-white hover:brightness-90"
                                style={{ backgroundColor: homeColor }}
                          ...
                          {taggingState === "Team2_Cover" && (
                            <div className="p-4" />
                          )}

                          {taggingState === "Team2_Attack" && (
                            <>
                              <button
                                type="button"
                                onClick={() => {
                                  handleStartTimelineTag("Dig", "home", 1);
                                  setTaggingState("G0_Cover");
                                }}
                                className="w-full py-3 px-4 text-left font-bold border-b border-black/20 cursor-pointer transition-colors text-white hover:brightness-90"
                                style={{ backgroundColor: homeColor }}
                          ...
                          {taggingState === "G0_Attack" && (
                            <>
                              <button
                                type="button"
                                onClick={() => {
                                  setCompletingTagData({
                                    actionName: "Attack Kill",
                                    teamName: homeTeamName,
                                    whoServed: "12",
                                    whoReceived: "",
                                    whoAttacked: "25",
                              ...
                              <button
                                type="button"
                                onClick={() => {
                                  handleStartTimelineTag("Set", "home", 0);
                                  setTaggingState("G0_Cover");
                                }}
                                className="w-full py-3 px-4 text-left font-bold border-b border-black/20 cursor-pointer transition-colors text-white hover:brightness-90"
                                style={{ backgroundColor: homeColor }}
                        ...
                        {/* Bottom Violation Button */}
                        <button
                          type="button"
                          onClick={() => {
                            setCompletingTagData({
                              actionName: "Violation",
                              teamName: homeTeamName,
                              whoServed: "1",
                              whoReceived: "",
                              serveRating: null,
                          ...
                          className="w-full py-3.5 px-4 text-left font-bold border-t border-black/20 cursor-pointer transition-colors text-white hover:brightness-90"
                          style={{ backgroundColor: homeColor }}
                        >
                          Violation
                        </button>
                    ...
                    <div
                      ...
                      style={{ backgroundColor: `${awayColor}38` }}
                    >
                      <div
                        className="py-2 px-4 text-xs font-extrabold uppercase text-white font-sans tracking-wider text-right flex-shrink-0"
                        style={{ backgroundColor: awayColor }}
                      >
                        {awayTeamName}
                      </div>
                      <div className="flex-grow flex flex-col justify-between">
                          ...
                          {taggingState === "G0_Serve" && (
                            <>
                              <button
                                type="button"
                                onClick={() => {
                                  handleStartTimelineTag("Serve Receive", "away", 1);
                                  setTaggingState("Team2_Serve_Receive");
                                }}
                                className="w-full py-3 px-4 text-right font-bold border-b border-black/20 cursor-pointer transition-colors text-white hover:brightness-90"
                                style={{ backgroundColor: awayColor }}
                              >
                                Serve Receive
                              </button>
                              <button
                                type="button"
                                onClick={() => {
                                  handleStartTimelineTag("Over Pass", "away", 1);
                                  setTaggingState("Team2_OverPass");
                                }}
                                className="w-full py-3 px-4 text-right font-bold border-b border-black/20 cursor-pointer transition-colors text-white hover:brightness-90"
                                style={{ backgroundColor: awayColor }}
                              >
                          ...
                          {taggingState === "Team2_Serve_Receive" && (
                            <>
                              <button
                                type="button"
                                onClick={() => {
                                  handleStartTimelineTag("Set #2", "away", 2);
                                  setTaggingState("Team2_Attack");
                                }}
                                className="w-full py-3 px-4 text-right font-bold border-b border-black/20 cursor-pointer transition-colors text-white hover:brightness-90"
                                style={{ backgroundColor: awayColor }}
                              >
                              ...
                              <button
                                type="button"
                                onClick={() => {
                                  handleStartTimelineTag("Free Ball", "away", 1);
                                  setTaggingState("Team2_FreeBall");
                                }}
                                className="w-full py-3 px-4 text-right font-bold border-b border-black/20 cursor-pointer transition-colors text-white hover:brightness-90"
                                style={{ backgroundColor: awayColor }}
                              >
                          ...
                          {taggingState === "G0_OverPass" && (
                            <>
                              <button
                                type="button"
                                onClick={() => {
                                  handleStartTimelineTag("Over Pass Receive", "away", 1);
                                  setTaggingState("Team2_Cover");
                                }}
                                className="w-full py-3 px-4 text-right font-bold border-b border-black/20 cursor-pointer transition-colors text-white hover:brightness-90"
                                style={{ backgroundColor: awayColor }}
                          ...
                          {taggingState === "G0_FreeBall" && (
                            <>
                              <button
                                type="button"
                                onClick={() => {
                                  handleStartTimelineTag("Free Ball Receive", "away", 1);
                                  setTaggingState("Team2_Cover");
                                }}
                                className="w-full py-3 px-4 text-right font-bold border-b border-black/20 cursor-pointer transition-colors text-white hover:brightness-90"
                                style={{ backgroundColor: awayColor }}
                          ...
                          {taggingState === "Team2_Attack" && (
                            <>
                              ...
                              <button
                                type="button"
                                onClick={() => {
                                  handleStartTimelineTag("Dig", "away", 1);
                                  setTaggingState("Team2_Cover");
                                }}
                                className="w-full py-3 px-4 text-right font-bold border-b border-black/20 cursor-pointer transition-colors text-white hover:brightness-90"
                                style={{ backgroundColor: awayColor }}
                        ...
                        <button
                          ...
                          className="w-full py-3.5 px-4 text-right font-bold border-t border-black/20 cursor-pointer transition-colors text-white hover:brightness-90"
                          style={{ backgroundColor: awayColor }}
                        >
                          Violation
                        </button>
                  ...
                  {/* Footer Actions: Problem Report & Save and Exit */}
                  <div className="flex flex-col gap-2 p-3.5 border-t border-neutral-900 bg-[#0f1317] flex-shrink-0">
                    <button
                      type="button"
